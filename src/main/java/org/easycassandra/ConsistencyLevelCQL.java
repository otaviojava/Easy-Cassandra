...
public enum ConsistencyLevelCQL {
    ...
    /**
     * Read
     * Will return the record returned by the first replica to respond.
     *  A consistency check is always done in a background thread to 
     *  fix any consistency issues when ConsistencyLevel.ONE is used. 
     *  This means subsequent calls will have correct data even if the 
     *  initial read gets an older value. (This is called ReadRepair) 
     * 
     * Write 
     * Ensure that the write has been written to at 
     * least 1 replica's commit log and memory table
     *  before responding to the client. 
     */
    ONE("CONSISTENCY ONE"),
    /**
     * Read
     *Will query all replicas and return the record with 
     *the most recent timestamp once it has at least a majority 
     *of replicas (N / 2 + 1) reported. Again, the remaining replicas
     ...
     * Read
     * Will query all replicas and return the record with
     * the most recent timestamp once all replicas have replied. 
     * Any unresponsive replicas will fail the operation. 
     * 
     * Write
     * Ensure that the write is written to all N replicas before 
     * responding to the client. Any unresponsive replicas will 
     * fail the operation. 
     */
    ALL("CONSISTENCY ALL"),
    /**
     ...
     *  once a majority of replicas within the local datacenter
     *  have replied. 
     * Write 
     ...
     * Read
     * Returns the record with the most recent timestamp
     *  once a majority of replicas within each datacenter have replied. 
     *  
     *  Write
